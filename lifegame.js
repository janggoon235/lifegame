 ...
 * 생명 게임에서 사용하는 객체
 *===========================================================================*/
const state    = Object.create(null);	// M : 생명 게임의 상태를 저장하는 객체
const view     = Object.create(null);	// V : 상태를 그래픽으로 표시하는 객체
const controls = Object.create(null);	// C : 컨트롤러 객체
//
...
window.onload = () => {
	console.log('start');
	readFile("./patterns.json", (jsonObj, error) => {
 ...
 * 생명 게임을 생성하는 함수
 *===========================================================================*/
const createLifeGame = (options) => {
	console.log('createLifeGame');
	const { parent, nx, ny, width, height } = options;
	// 타이틀
	const title = elt("h1",{ class: "title" },"Life Game");
	// view 객체를 생성한다 (반환값은 뷰 패널)
	const viewpanel = view.create(nx, ny, width, height);
	// state 객체를 초기화한다
	state.create(nx, ny);
...
state.tellGenerationChange = (generation) => {
...
state.update = () => {
	...
	for(let ix=0; ix<state.nx; ix++) {
		for(let iy=0; iy<state.ny; iy++) {
			const sum = state.getSumAround(ix,iy);
...
view.create = (nx, ny, width, height) => {
	...
	view.layer[1].addEventListener("click", (e) => {
		const ix = Math.floor(e.offsetX/view.cellWidth);	// 셀의 x방향 번호
		const iy = Math.floor(e.offsetY/view.cellHeight);	// 셀의 y방향 번호
...
view.drawCell = (ix, iy, life) => {
	const c = view.ctx[0];	// 생물은 layer[0]에 그린다
	c.beginPath();
	if (life) {	// 표식 (원)을 그린다
		const x = (ix+0.5)*view.cellWidth;
		const y = (iy+0.5)*view.cellHeight;
		const r = view.markRadius;
		
		c.fillStyle = view.markColor;
		c.arc(x,y,r,0,Math.PI*2,true);
		c.fill();
	} else {	// 셀에 배경색을 칠한다
		const x = ix*view.cellWidth;
		const y = iy*view.cellHeight;
		c.fillStyle = view.backColor;
		c.fillRect(
			x,
			y,
...
view.showGeneration = (generation) => {
...
controls.play = (state) => {
	...
	const input = elt("input",{ type: "button", value: "연속 재생" });
...
// * 재생 속도 설정 메뉴
controls.changeTimeInterval = (state) => {
	...
	for(let i=0; i<options.length; i++) {
		const option = elt("option",null,options[i].name);
...
// * 정지 버튼
controls.stop = (state) => {
	const input = elt("input", { type: "button", value: "정지" });
...
controls.step = (state) => {
	const input = elt("input", { type: "button", value: "다음" });
...
// * 패턴 선택 메뉴
controls.pattern = (state) => {
	const select = elt("select");
	const placePattern = (pattern) => {
		const array = pattern.points;
		// x, y의 최솟값과 최댓값을 구한다
		const max = [0,0];
		const min = [state.nx-1,state.ny-1];
		...
		for(let i=0; i<array.length; i++) {
			const ix = array[i][0]+Math.floor((state.nx-min[0]-max[0])/2);
			const iy = array[i][1]+Math.floor((state.ny-min[1]-max[1])/2);
...
controls.clear = (state) => {
	const input = elt("input",{ type: "button", value: "모두 삭제" });
